...
class _GroceryListState extends State<GroceryList> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder(
        future: _loadedItems, // don't want _loadedItems() because it will execute every time we call build method
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }

          if (snapshot.hasError) { // will be the exception we defined when statuscode >= 400
            return Center(
              child: Text(
